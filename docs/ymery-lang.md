# Ymery YAML Language Guide

Ymery is a declarative UI framework for Dear ImGui using YAML. This guide covers the complete YAML-based language for building ImGui applications.

## File Structure

A complete Ymery application consists of YAML files with four top-level sections:
...
This section explains how Ymery GUI elements interact with Python code through the DataBag interface.

### The DataBag Interface

Every widget in Ymery accesses data through a **DataBag** object. DataBag provides a unified interface for reading and writing values, regardless of where the data comes from.
